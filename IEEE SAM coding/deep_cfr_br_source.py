...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    self._policy_network = [
        torch.load(self.network0),torch.load(self.network1)
    ]
  ...
  def solve(self):
    ...
    torch.save(self._policy_network[1],self.save_path)
    ...
    if state.is_terminal():
      ...
      for action in range(limitation):
        if state.current_player() == 0:
          if action >= 2:
            break
          strategy = self.action_probabilities(state, player)
          probs = np.array(strategy)
          probs /= probs.sum()
        else:
          strategy = [0 for _ in range(27)]+[0.2 for _ in range(5)] # Best Response to fix one player
          probs = np.array(strategy)
      ...
      Jammer_action = [27,28,29,30,31]
      if other_player == 0:
        strategy = self.action_probabilities(state, other_player)
      else:
