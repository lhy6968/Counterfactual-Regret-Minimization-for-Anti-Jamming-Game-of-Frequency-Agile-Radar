...
class DQN(rl_agent.AbstractAgent):
  ...
  def step(self, time_step, is_evaluation=False, add_transition_record=True):
    ...
    if (not time_step.last()) and (
      ...
      if len(info_state) == 0:
        if self.player_id == 1:
            sampled_action = np.random.choice([27,28,29,30,31])
            return rl_agent.StepOutput(action=sampled_action, probs=[0.2 for _ in range(5)])
        else:
            sampled_action = np.random.choice([n for n in range(27)])
            return rl_agent.StepOutput(action=sampled_action, probs=[1/27 for _ in range(27)])
    ...
    if not is_evaluation:
      ...
      if self.player_id == 1:
        sampled_action = np.random.choice([30])
        return rl_agent.StepOutput(action=sampled_action, probs=[0 for _ in range(27)]+[0,0,0,1,0])
  ...
  def _epsilon_greedy(self, info_state, legal_actions, epsilon):
    ...
    if np.random.rand() < epsilon:
      ...
      q_values = self._q_network(info_state)
  ...
  def learn(self):
    ...
    # print(transitions)
    info_states = torch.Tensor([t.info_state.numpy() for t in transitions])
    # info_states = transitions
    actions = torch.LongTensor([t.action for t in transitions])
    rewards = torch.Tensor([t.reward for t in transitions])
    next_info_states = torch.Tensor([t.next_info_state.numpy() for t in transitions])
