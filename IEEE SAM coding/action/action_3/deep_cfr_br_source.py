...
from Embedding import boardTensor
import pyspiel
...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
               ...
               advantage_network_train_steps: int = 300,
    ...
    self._embedding_size = len(boardTensor()._board_to_string(self._root_node.board))
    # print(self._embedding_size)
  ...
  def solve(self):
    ...
    policy_loss = self._learn_strategy_network(1)
    torch.save(self._policy_network[0],"policy_network_action3_version2.pkl")
    # torch.save(self._advantage_networks[0],"network/advantage_network_0.pkl")
    # torch.save(self._advantage_networks[1],"network/advantage_network_1.pkl")
  ...
  def _traverse_game_tree(self, state, player):
    ...
    if state.is_terminal():
      ...
      Radar_action = [n for n in range(27)]
      Jammer_action = [27,28,29,30,31]
      sampled_regret = collections.defaultdict(float)
      limitation = 5
      # Update the policy over the info set & actions via regret matching.
      for action in range(limitation):
        if state.current_player() == 0:
          strategy = [1/27 for _ in range(27)]+[0 for _ in range(5)]
          probs = np.array(strategy)
          # probs /= probs.sum()
        else:
          strategy = [0 for _ in range(27)]+[0,0.5,0.5,0,0]
          # _, strategy = self._sample_action_from_advantage(state, 1)
          probs = np.array(strategy)
          probs /= probs.sum()
          # probs = self.exploration_strategy(probs,0.9)
          # print(probs,"Jammer_new")
        sampled_action = np.random.choice(range(self._num_actions), p=probs)
        if (state.current_player() == 0) and (sampled_action not in Radar_action):
          print("Radar Policy",probs)
          print(state.board)
          while True:
              sampled_action = np.random.choice(range(self._num_actions), p=probs)
              if sampled_action in Radar_action:
                  break
        if (state.current_player() == 1) and (sampled_action not in Jammer_action):
            print("Jammer Policy",probs)
            print(state.board)
            while True:
                sampled_action = np.random.choice(range(self._num_actions), p=probs)
                if sampled_action in Jammer_action:
                    break
      # action_list = state.legal_actions().copy()
      ...
      Jammer_action = [27,28,29,30,31]
      if other_player == 1:
        strategy = strategy = [0 for _ in range(27)]+[0,0.5,0.5,0,0]
      else:
      ...
      sampled_action = np.random.choice(range(self._num_actions), p=probs)
      if (other_player == 0) and (sampled_action not in Radar_action):
          print("Radar Policy",probs)
          print(state.board)
          while True:
              sampled_action = np.random.choice(range(self._num_actions), p=probs)
              if sampled_action in Radar_action:
                  break
      if (other_player == 1) and (sampled_action not in Jammer_action):
          print("Jammer Policy",probs)
          print(state.board)
          while True:
              sampled_action = np.random.choice(range(self._num_actions), p=probs)
  ...
  def exploration_strategy(self,probs,epsilon):
    probs /= probs.sum()
    probs = probs*(1-epsilon)
    n = epsilon/5
    probs = probs + n
    for i in range(27):
      probs[i] = 0
  ...
  def action_probabilities(self, state,player):
    ...
    with torch.no_grad():
      # print(info_state_vector[0])
      logits = self._policy_network[player](torch.FloatTensor(info_state_vector))
